"""Implémentations ``plot_*`` du service graphique Pacing.

Les méthodes vivent dans ``GraphPlotsMixin`` ; ``ServiceGraphe`` les hérite
pour conserver l'API publique inchangée.
"""
...
class GraphPlotsMixin:
    """Mixin regroupant toutes les méthodes ``plot_*`` de ``ServiceGraphe``.

    Attributes:
        Aucun attribut propre : s'appuie sur les caches et helpers de
            ``ServiceGraphe`` (``_get_cached_*``, etc.).
    """
