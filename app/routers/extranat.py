...
from app.services.extranat_service import get_all_results_by_type, get_results_by_date_range
...
async def get_all_results():
    ...
    data = await asyncio.get_event_loop().run_in_executor(
        None, lambda: get_all_results_by_type(delay_between_comps=0.5, debug=False)
    )
    types_list = data.get("types", [])
    n_comp = sum(len(t.get("competitions", [])) for t in types_list)
    return {"count_types": len(types_list), "count_competitions": n_comp, "data": data}
...
async def get_results_by_type(type_competition: TypeCompetitionLabel = Query(..., alias="type_competition")):
    ...
    data = await asyncio.get_event_loop().run_in_executor(
        None, lambda: get_all_results_by_type(delay_between_comps=0.5, debug=False, only_idtyps=[idtyp])
    )
