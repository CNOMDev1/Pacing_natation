...
Centralise ``PROJECT_DIR`` et les dossiers ``data/``, secrets et exports pour
éviter les ``Path(__file__).parents[N]`` dispersés dans les loaders et scripts.
"""
...
# pacing/config/paths.py → parents[2] = racine du dépôt
...
OMEGA_PROCESSED_DIR = PROCESSED_DIR / "omega"
...
# Secrets legacy (toujours sous services/ pour ne pas casser les workflows locaux)
...
def ensure_project_imports() -> Path:
    ...
    Utile pour les lancements directs de scripts sans ``pip install -e .``.
