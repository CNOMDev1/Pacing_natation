...
def normalize_name(value: object) -> str:
    ...
    Alias sémantique de ``normalize_text`` (compatibilité couloirs).
...
def solo_swimmer_dict(swimmers: Any) -> Optional[dict]:
    """
    Extrait le dict nageur si la performance est solo (un seul élément).

    Args:
        swimmers (Any): Valeur de la colonne ``swimmer``.

    Returns:
        Optional[dict]: Dictionnaire nageur ou None.
    """
    if not isinstance(swimmers, list) or len(swimmers) != 1:
        return None
    first = swimmers[0]
